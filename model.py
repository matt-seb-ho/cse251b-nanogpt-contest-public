...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Precompute AliBi slopes, shape (1, n_head, 1, 1)
        head_indices = torch.arange(1, self.n_head + 1)
        slopes = -(2 ** (-8.0 / self.n_head * head_indices))
        self.register_buffer('slopes', slopes.view(1, self.n_head, 1, 1))
...
class SwiGLUFFN(nn.Module):
    def __init__(self, config):
        super().__init__()
        hidden = round(8/3 * config.n_embd / 64) * 64  # 2048 for n_embd=768
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        muon_params = [p for n, p in self.named_parameters()
                       if p.requires_grad and p.ndim==2
                       and not n.startswith('transformer.wte')
                       and not n == 'lm_head.weight']
        
        adamw_params = [p for n, p in self.named_parameters()
                    if p.requires_grad and (p.ndim < 2
                    or n.startswith('transformer.wte')
                    or n == 'lm_head.weight')]

        muon_optimizer = Muon(muon_params, lr=0.02, momentum=0.95)
        adamw_optimizer = torch.optim.AdamW(adamw_params, lr=learning_rate, betas=betas,
